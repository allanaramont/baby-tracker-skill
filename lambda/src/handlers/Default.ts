...
import { supportsAPL, criarHomeDashboardAPL, criarTimerAmamentacaoAPL } from '../utils/apl';
import { tempoDecorrido, formatarFralda, formatarTempo, formatarLado } from '../utils/tempo';
import { temContaVinculada } from '../utils/auth';
...
export const LaunchRequestHandler: RequestHandler = {
  ...
  async handle(handlerInput: HandlerInput) {
    ...
    if (!temContaVinculada(handlerInput.requestEnvelope)) {
    ...
    if (estado.mamadaAtual?.emAndamento) {
      ...
      const ladoStr = formatarLado(lado);
...
export const FallbackIntentHandler: RequestHandler = {
  ...
  async handle(handlerInput: HandlerInput) {
    ...
    if (estado.mamadaAtual?.emAndamento) {
      const decorrido = tempoDecorrido(estado.mamadaAtual.iniciadaEm);
      const ladoStr = formatarLado(estado.mamadaAtual.lado ?? 'esquerdo');
      return handlerInput.responseBuilder
