...
const TrocarFralda_1 = require("./TrocarFralda");
...
describe('TrocarFraldaHandler', () => {
    describe('canHandle', () => {
        it('aceita intent TrocarFralda', () => {
            ...
            expect(TrocarFralda_1.TrocarFraldaHandler.canHandle(handlerInput)).toBe(true);
        });
        it('rejeita outros intents', () => {
            const { handlerInput } = criarMockHandlerInput({});
            handlerInput.requestEnvelope.request.intent.name = 'OutroIntent';
            expect(TrocarFralda_1.TrocarFraldaHandler.canHandle(handlerInput)).toBe(false);
        });
    ...
    describe('handle - sem slot tipoBaixo', () => {
        it('solicita o slot tipoBaixo quando ausente', async () => {
            ...
            await TrocarFralda_1.TrocarFraldaHandler.handle(handlerInput);
    ...
    describe('handle - com slot xixi', () => {
        it('registra fralda de xixi e retorna mensagem correta', async () => {
            const { handlerInput, estado, setPersistentAttributes, mocks } = criarMockHandlerInput({ slotId: 'xixi' });
            await TrocarFralda_1.TrocarFraldaHandler.handle(handlerInput);
            expect(mocks.speak).toHaveBeenCalledWith(expect.stringContaining('só xixi'));
            expect(mocks.speak).toHaveBeenCalledWith(expect.stringContaining('Fralda com'));
        });
        it('salva os atributos persistentes', async () => {
            const { handlerInput, savePersistentAttributes, setPersistentAttributes } = criarMockHandlerInput({ slotId: 'xixi' });
            await TrocarFralda_1.TrocarFraldaHandler.handle(handlerInput);
        ...
        it('adiciona fralda ao início da lista ultimasFraldas', async () => {
            ...
            await TrocarFralda_1.TrocarFraldaHandler.handle(handlerInput);
            const estadoSalvo = setPersistentAttributes.mock.calls[0][0];
